            ...
            for text_file in os.listdir(space_path):
                ...
                with open(text_path, "r", encoding="utf-8") as file:
                    ...
                    if embeddings is not None:
                        # Save the embeddings
                        metadata = {"space_key": space_dir, "page_id": text_file.replace(".txt", "")}
                        print(f"metadata is: {metadata}")
                        try:
                            chroma_store.add_embeddings([text_file.replace(".txt", "")],embeddings.tolist(), [metadata])  # Note: single embedding, hence the list wrapping
                        except Exception as e:
